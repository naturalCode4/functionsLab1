...
console.log(exclaimThree('hOoRAy'))
...
function nameCheck(name) {
  if (name == 'Steven') {
    ...
    return 'Hey Bryan!'
  } else return `Cool name, ${name.toUpperCase()}`
}

function nameCheck2(name) {
   return name == 'Steven' ? 'What is up Steven?'
  : name == 'Bryan' ? 'Hey Bryan!'
  : `Cool name, ${name.toUpperCase()}`
}

console.log(nameCheck2('Steven'))
console.log(nameCheck2('Bryan'))
console.log(nameCheck2('Adin'))
...
colorRating = faveColorFinder('orange')
...
const bigOrSmall = arr => {
  ...
  for (let x = 0; x<arr.length; x++) {
    arr[x] > 100 ? arrayEvaluator.push('BIG') : arrayEvaluator.push('small')
  }
...
let loser = 'Glimmer'
let losers = ['Glimmer', 'Peeta', 'Clove', 'Fox-face','Cato','Barack Obama']
/*
...
const theEliminator = (people, dead) => {
...
const theEliminator2 = (people, dead) => { // Another way of writing the function
  if (people.includes(dead)) {
    people.splice(people.indexOf(dead),1) }
  return people
}

// console.log(theEliminator2(contestants, loser))

const theEliminator3 = (people, dead) => { // This function is able to take multiple losers. Note the loserS category above
  for (let i=0; i<people.length; i++) {
...
console.log(theEliminator3(contestants, losers))
...
*/
let myEmail = '    akramer@wesleyan.edu   '
...
const checkMyEmail = emailCheck(myEmail)
